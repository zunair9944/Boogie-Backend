...
class ChatifyMessenger implements HasMedia
{
    public $pusher;
    ...
    {
        Log::info("Triggered Notification trigger");
     ...
     */
    // $requestUser,
    public function pusherAuth( $authUser, $channelName, $socket_id)
    {
        // Auth data
    ...
    {
        return $this->fetchMessagesQuery($user_id)->latest();
    }
    ...
    {
        // get last message
        $lastMessage = $this->getLastMessageQuery($user->id)->orderBy('created_at','DESC')->first();
        // Get Unseen messages counter
        $unseenCounter = $this->countUnseenMessages($user->id);
        // dd($this->getUserWithAvatar($user));
                if( request()->is('api/*'))
                {
                    $user12 = \DB::table('users')->where('id',$user->id)->first();
                    $image = \DB::table('uploads')->where('id',$user12->profile_image_id)->first();
                   
                    if(isset($image))
                    {
                        $profileImage =  asset($image->file_name);
                    }
                    else
                    {
                        $profileImage =  null;
    ...
    public function getUserWithAvatar($user)
    {
        ...
        // dd($mediaItems);
    //    dd( getSingleMedia($user, 'document_image', null));
        if ($user->avatar == 'avatar.png' && config('chatify.gravatar.enabled')) {
            ...
            $user->avatar =  getSingleMedia($user, '_image', null);
        }
        return $user;
    }
