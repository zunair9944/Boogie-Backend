...
use Stripe\Token;
 ...
 {
       $upload = Upload::where('id',$user->profile_image_id)->first();
        $image = isset($upload->file_name) ? str_replace("public/", "", $upload->file_name) : null;
        if(isset($image))
        {
            $profileImage =  isset($image) ? asset($image) : null;
        }
        else
        {
            $profileImage =  null;
...
function chargeCard($card_id,$amount)
{
    $card = Card::where('id', $card_id)->first();
    if(isset($card))
    {
        $time = strtotime($card->expiration_date);
        $month = date("m", $time);
        $year = date("Y", $time);
        $key = "sk_test_51MTR37LCgmPqABZjzXFsyMsQMPwJGr2dpB45BuvZ7JIfx2O87FNto2urhxXI9i3HoYSbJ6oJY105P83nKQIS8qjx002lBn41lf";
...
function getData($user_riderequest_detail)
{
    $nearby_driver = User::with('riderDetail')->where('id', $user_riderequest_detail->driver_id)->first();
     if(!isset($nearby_driver))
    {
        return;
    }
    $rider = User::with('riderDetail')->where('id', $user_riderequest_detail->rider_id)->first();
    $rider_cards = Card::where('rider_id', $user_riderequest_detail->rider_id)->where('default', true)->first();
    // dump($nearby_driver);
    // dd(getImagePath($nearby_driver));
    $driverProfileImage = getImagePath($nearby_driver); //getSingleMedia($nearby_driver, 'profile_image');
    $riderProfileImage = getImagePath($rider); //getSingleMedia($rider, 'profile_image');
    $start_latitude = $user_riderequest_detail->start_latitude;
    $start_longitude = $user_riderequest_detail->start_longitude;
    $end_latitude = $user_riderequest_detail->end_latitude;
    $end_longitude = $user_riderequest_detail->end_longitude;
    $data =  distance($start_latitude, $start_longitude, $end_latitude, $end_longitude);
    $riderPickup =  distance($start_latitude, $start_longitude, $nearby_driver->latitude, $nearby_driver->longitude);
    $riderequest = RideRequest::with('service')->find($user_riderequest_detail->id);
    // Request time Data for Ride History Record
    $requestRating = $user_riderequest_detail->rideRequestRating()->first();
    if(date('Y-m-d') ==   $user_riderequest_detail->created_at->format('Y-m-d'))
        {
            $v = $user_riderequest_detail->created_at->diffForHumans();
            if (str_contains($v,'minutes')) { 
                $ride_request_time = str_replace(['minutes'],['Minutes'],$v);
            }
            if(str_contains($v,'minute'))
            {
                $ride_request_time = str_replace(['minute'],['Minute'],$v);
            }  
            if(str_contains($v,'hour'))
            {
                $ride_request_time = str_replace(['hour'],['Hour'],$v);
            }
            if(str_contains($v,'seconds'))
            {
                $ride_request_time = str_replace(['seconds'],['Seconds'],$v);
            }

            // $ride_request_time =  $user_riderequest_detail->created_at->diffForHumans();
        }
        else
        {
            $ride_request_time =  $user_riderequest_detail->created_at->format('d M h:i A');
        }
          // End Request time Data for Ride History Record

    if (isset($riderequest->service))
        {
            $carInformation =  $riderequest->service->first();
           $service_image = getSingleMedia($riderequest->service, 'service_image',null);
        }
    else
        $carInformation = [];

       $ride_image = Upload::where('id',$user_riderequest_detail->ride_image_id)->first();
    $riderOrDriverDetail = [
        
        'requestId' => $user_riderequest_detail->id,
        'ride_image' => (isset($ride_image)) ? asset($ride_image->file_name) : null,
        'riderequest_inner_date' => $user_riderequest_detail->created_at->format('d M Y'),
        'ride_status' => $user_riderequest_detail->status,
        'request_rating' => isset($requestRating) ? (int)$requestRating->rating : 0,
        'ride_request_time' => $ride_request_time,
        'rider_id' => $user_riderequest_detail->rider_id,
        'driver_id' => $nearby_driver->id,
        'base_fare' => $user_riderequest_detail->base_fare,
        'distance'  => $data['distance'],
        'dropoff_address' => $data['destination_addresses'][0],
        'pickup_address' => $data['origin_addresses'][0],
        'pickup_distance' => $riderPickup['distance'],
        'start_time' => $user_riderequest_detail->start_time,
        'end_time' => $user_riderequest_detail->end_time,
        'start_latitude' => $user_riderequest_detail->start_latitude,
        'start_longitude' => $user_riderequest_detail->start_longitude,
        'end_latitude' => $user_riderequest_detail->end_latitude,
        'end_longitude' => $user_riderequest_detail->end_longitude,
        'time' =>  $data['duration'],
        'fare'  => "RS." . ($user_riderequest_detail->base_fare + round((int)$data['distance'] * 8)),
        'riderequest_in_datetime' => date('Y-m-d',strtotime($user_riderequest_detail->created_at)),
        "driverData" => [
            'requestId' => $user_riderequest_detail->id,
            'name' => $nearby_driver->full_name,
            'phone' => $nearby_driver->contact_number,
            'profile_image' => $driverProfileImage,
            'ratings' => $nearby_driver->driverRating()->count(),
            'latitude' => $nearby_driver->latitude,
            'logitude' => $nearby_driver->longitude,
        ],
        "riderData" => [
            'requestId' => $user_riderequest_detail->id,
            ...
            'profile_image' => $riderProfileImage,
            'ratings' => $nearby_driver->riderRating()->count(),
            'latitude' => $rider->latitude,
            'logitude' => $rider->longitude,
        ],
        'cardInfo' => (isset($rider_cards->card_number)) ? getTruncatedCCNumber($rider_cards->card_number) : '------',
        "carInformation" => [
            'name' => $carInformation->name ?? '',
            'licence_plate' => $carInformation->licence_plate ?? '',
            'base_fare' => $carInformation->capacity ?? '',
            'image' =>   $service_image ?? '',
        ]
    ];

    return $riderOrDriverDetail;
}
...
function getTruncatedCCNumber($ccNum)
{
    ...
    return  substr($ccNum, -4);
}
//calculate information on basis of longitude and latitude
function  distance($start_latitude, $start_longitude, $end_latitude, $end_longitude)
{
    $url = 'https://maps.googleapis.com/maps/api/distancematrix/json?destinations=' . $end_latitude . '%2C' . $end_longitude . '%7C&origins=' . $start_latitude . '%2C' . $start_longitude . '&key=AIzaSyCxpgHzjtRrqpdIB-Rm_ZsLA8lR1F4bSrI';
    $json = file_get_contents($url);
    $address = json_decode($json);
    $data['destination_addresses'] = $address->destination_addresses;
    $data['origin_addresses'] = $address->origin_addresses;
    $data['duration'] = $address->rows[0]->elements[0]->duration->text;
    $data['distance'] = $address->rows[0]->elements[0]->distance->text;
    // $url = 'https://maps.googleapis.com/maps/api/directions/json?origin='. $data['destination_addresses'][0].'&destination='. $data['origin_addresses'][0].'&key=AIzaSyDs-RA9UNefvlGG5868vLUFR6A5frTfnAs';
    // dd($url);
    // $json = file_get_contents($url);
    // $address = json_decode($json);
    // dd($address);
...
function getDriverRideRequestNotificationData($ride_request, $nearby_driver)
{
    ...
    // Duration and Distance estimate of Ride Request
    $data =  distance($start_latitude, $start_longitude, $end_latitude, $end_longitude);
    // Duration and Distance estimate between Nearby Driver and pickup Address
    $riderPickup =  distance($start_latitude, $start_longitude, $nearby_driver->latitude, $nearby_driver->longitude);
    $rider = User::where('id', $ride_request->rider_id)->first();
    $riderProfileImage = getSingleMedia($rider, 'profile_image');
    $driverProfileImage = getSingleMedia($nearby_driver, 'profile_image');
    $notification_data = [
        ...
        'end_latitude' => $ride_request->end_latitude,
        'end_longitude' => $ride_request->end_longitude,
        'dropoff_distance'  => $data['distance'],
        'dropoff_address' => $data['destination_addresses'][0],
        'pickup_address' => $data['origin_addresses'][0],
        'pickup_distance' => $riderPickup['distance'],
        'time' =>  $data['duration'],
        'fare'  => "RS." . ($ride_request->base_fare + round((int)$data['distance'] * 8)),
        'riderequest_in_datetime' => \Carbon\Carbon::now()->format('Y-m-d H:i:s'),
        "driverData" => [
        ...
        "riderData" => [
            ...
            'ratings' => $rider->riderRating->count(),
        ]
    ];
    return $notification_data;
}
function getNearByDrivers($ride_request)
{
    ...
    $unit_value = convertUnitvalue($unit);
    $nearby_drivers = User::whereHas('userDetail',function($query) use($ride_request){
           return   $query->where('car_category_id',$ride_request->service_id);
    })->selectRaw("id,full_name,first_name,last_name,user_type, player_id, latitude, longitude, 
    ...
    if (getFileExistsCheck($media)) {
        ...
        switch ($collection) {
            ...
            case 'site_favicon':
                ...
                break;
            case '_image':
                $media = asset('images/person.png');
                break;
            default:
                $media = asset('images/vigo.png');
                break;
        }
        return $media;
    }
